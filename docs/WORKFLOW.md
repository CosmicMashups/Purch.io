...
A business can also set up **automatic promos** that need no cashier action to apply: a **Buy 1 Take 1** rule (buy one item, get one free — the same item or a different one), a **combo bundle** (two specific items priced as one flat total when bought together, e.g. ₱85 for a burger + a drink), or a **time-boxed item discount** (percent off, a fixed amount off, or a flat sale price for a specific item, active only during a set window). All three apply themselves the instant a qualifying item lands in the cart — no code to enter, no button to press — and stack underneath the existing Senior/PWD discount and any promo code.

For a café or restaurant, what's sold at the register ("Americano," "Adobo") often isn't what's tracked in the stockroom (coffee beans, milk, soy sauce, garlic). An owner can turn on **ingredient-level inventory tracking** in Business Settings to separate the two: ingredients get their own stock records with a unit of measure and delivery packaging (e.g. a bottle of soy sauce is "450 mL," delivered "10 pcs" at a time), and a menu item's **recipe** lists which ingredients it uses — with an optional amount per order. Leave the amount blank and the system just checks the ingredient has *any* stock left before marking the item sellable; fill it in and every sale automatically deducts that amount, rolling over into the next delivered package once one runs out, with no manual recount needed. A menu item with no recipe configured keeps working exactly as it always did — the owner manually updates its own Physical Count.

## 3. A Typical Sale (Cashier)

The cashier logs into a paired device with their PIN and opens a fresh cart. From there:

1. **Add items** — tap a unit-priced item to add it directly; a variant-matrix item (clothing, department store) opens a size/color picker first; a combo item (café) opens a slot-by-slot customization sheet; other pricing types route to their own entry flow. Whatever was picked — the chosen size/color, combo slot selections, modifier add-ons — stays visible right on the cart line, not just buried in the total.
2. **Automatic promos apply themselves** — if the owner has an active Buy-1-Take-1, combo-bundle, or item-discount promo running (see §2), the matching cart line(s) reprice automatically the moment the qualifying items are in the cart, with a small tag on the line (e.g. "BUY 1 TAKE 1," "COMBO ₱85.00") showing why. The cashier does nothing to trigger this — no code to type, no button to press — and it stacks underneath the discounts below.
3. **Apply discounts** — toggle the Senior Citizen/PWD 20% discount only after physically verifying the customer's ID (this is a manual, staff-verified action, never automatic), or enter a promo code if the business runs one.
4. **Choose fulfillment**, where relevant (e.g. dine-in vs. take-out for a café).
5. **Charge** — pick a payment method: **Cash** (system computes change), **Bank Transfer** (cashier confirms manually after checking the transfer landed), **Manual GCash QR** (a merchant-printed static QR code, same manual-confirmation trust model), or **Utang/Credit** (charges a returning customer's account instead of collecting payment now — see §8). Digital-gateway QR Ph, e-load/bill payment, and split-tender payments are recognized by the system but intentionally not available yet — the app tells the cashier why rather than pretending to accept them.
6. **Receipt** — on success, the sale gets a sequential, gap-auditable receipt number and a receipt is shown; the cashier's next action is always "New Sale," so there's no way to accidentally back into a completed sale's cart.

Stock decrements automatically in the background the moment a sale completes, logged as its own **Sale** movement distinct from a manual stock-out or a recipe's ingredient consumption — nobody manually reconciles the register against the shelf count after the fact, and the movement log always shows why stock changed.

## 4. The Self-Service Kiosk (Customer, unattended)

For verticals that support it — most naturally a café, but usable anywhere the owner wants a self-order station — a second, portrait-orientation terminal lets customers build their own order without a cashier: a welcome screen, a category carousel, the same combo/variant customization sheets a cashier would use, a cart review showing everything they picked (same per-line detail as the cashier's cart), a fulfillment choice, and a submitted order with a short **prep number** the customer takes to the counter.
...
- **Ingredient stock (where enabled, §2)** — an Inventory Item's quantity is tracked in one base unit (e.g. mL) but delivered and displayed in packages (e.g. "10 pcs" of a 450 mL bottle); receiving a delivery adds whole packages, and every recipe-linked sale draws down the same running total continuously, rolling into the next package automatically once one is used up — no separate "package count" to keep in sync by hand.
