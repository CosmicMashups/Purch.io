...
| G32 | RESOLVED | The tab icon and touch icon are now made from the logo (`web/public/favicon-32.png`, `apple-touch-icon.png`, `icon-192.png`). Images follow the Flutter client: stored values may be `assets/...` bundled paths, `/uploads/...` or URLs, resolved by `lib/images.ts`. Only the logo, wordmark, kiosk poster and three sample pictures are bundled; `login_hero.jpg` is unused, as in Flutter. |
...
1. **Name the app "Purch.io". DONE.** The package is `purch.io` (`npm run dev` prints `purch.io@0.0.0 dev`) and the page title and description say Purch.io. The tab icon comes from the logo, see G32.
